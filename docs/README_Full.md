...
- **树形题库管理生态**：`bank_node` 邻接表树（`FOLDER` / `LEAF` 任意深度），管理端拖拽排序；大厅仅展示公开根节点；挂题/刷题/AI 导入仅 `LEAF`。
- **完善的题库 CRUD 与兼容层**：`/api/v1/bank-nodes` 为主 API；`/api/v1/question-banks` 保留根级 LEAF 映射（已废弃）。
